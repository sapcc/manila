...
class NetAppCmodeClient(client_base.NetAppBaseClient):
    ...
    def configure_dns(self, security_service):
        ...
        dns_ips = [x.strip() for x in security_service['dns_ip'].split(',')]
        if len(dns_ips) == 1:
            api_args['name-servers'] = {'ip-address': dns_ips[0]}
        else:
            for ip in dns_ips:
                api_args['name-servers'].append({'ip-address': ip})
    ...
    def set_preferred_dc(self, security_service):
        ...
        dc_ips = [x.strip() for x in security_service['server'].split(',')]
        for ip in dc_ips:
            api_args['preferred-dc'].append({'string': ip})
    ...
    def create_volume(self, aggregate_name, volume_name, size_gb,
                      ...
                      qos_policy_group=None, comment='', **options):
    ...
    def modify_volume(self, aggregate_name, volume_name,
                      ...
                      language=None, dedup_enabled=False,
                      compression_enabled=False, max_files=None,
                      qos_policy_group=None, comment=None, replica=False,
                      **options):
