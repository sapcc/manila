...
class NetAppCmodeClient(client_base.NetAppBaseClient):
    ...
    def _init_features(self):
        ...
        ontap_9_10_1 = system_version['version-tuple'] >= (9, 10, 1)
        ontap_9_11_1 = system_version['version-tuple'] >= (9, 11, 1)
        ontap_9_12 = system_version['version-tuple'] >= (9, 12, 0)
        ontap_9_12_1 = system_version['version-tuple'] >= (9, 12, 1)
    ...
    def _create_vserver(self, vserver_name, aggregate_names, ipspace_name,
        ...
        self.modify_vserver(vserver_name, aggregate_names,
                            delete_retention_hours)
    ...
    def modify_vserver(self, vserver_name, aggregate_names, retention_hours):
        ...
        if self.features.DELETE_RETENTION_HOURS:
            modify_args['volume-delete-retention-hours'] = str(retention_hours)
    ...
    def configure_cifs_aes_encryption(self, aes_encryption, secure=True):
        ...
        if not secure:
            ...
            api_args['use-ldaps-for-ad-ldap'] = 'true'
            api_args['session-security-for-ad-ldap'] = 'sign'
    ...
    def create_volume(self, aggregate_name, volume_name, size_gb,
        ...
        size_b = self._calculate_volume_size(
            size_gb, options.get('provision_net_capacity', False),
            snapshot_reserve)
        ...
        api_args.update(self._get_create_volume_api_args(
            ...
            unix_permissions, logical_space_reporting))
    ...
    def _calculate_volume_size(self, size_gb, provision_net_capacity=False,
                               snapshot_reserve=None):
        """Return volume size in bytes, adjusted for net capacity if needed."""
        if provision_net_capacity and snapshot_reserve is not None:
            return int(size_gb * units.Gi * 100 / (100 - snapshot_reserve))
        return int(size_gb * units.Gi)
    ...
    def _get_create_volume_api_args(self, volume_name, thin_provisioned,
        ...
        if logical_space_reporting in (True, False):
            api_args['is-space-reporting-logical'] = (
                'true' if logical_space_reporting else 'false')
            api_args['is-space-enforcement-logical'] = (
                'true' if logical_space_reporting else 'false')
    ...
    def set_volume_max_files(self, volume_name, max_files,
        ...
        if not allow_decrease:
            ...
            current_max_files = alloc_files['max'] if alloc_files else None
    ...
    def set_volume_size(self, volume_name, size_gb, **options):
        ...
        provision_net_capacity = options.get('provision_net_capacity', False)

        size_b = self._calculate_volume_size(
            size_gb, provision_net_capacity, snapshot_reserve_percent)
    ...
    def get_volume(self, volume_name):
        ...
        volume = {
            ...
            'size-used': volume_space_attributes.get_child_content(
                'size-used'),
            'size-total': volume_space_attributes.get_child_content(
                'size-total'),
    ...
    @na_utils.trace
    def volume_online(self, volume_name):
        api_args = {'name': volume_name}
        self.send_request('volume-online', api_args)

    @na_utils.trace
    def volume_offline(self, volume_name):
        api_args = {'name': volume_name}
        self.send_request('volume-offline', api_args)
