...
class NetAppCmodeMultiSVMFileStorageLibrary(
    ...
    def _get_or_create_ipspace(self, network_info, client=None):
        ...
        desired_client = client or self._client
        ...
        # the network_type from the network_info dict itself
    ...
    def create_share(self, context, share, share_server):
        """Enforce SMas protection on reused share servers."""
    ...
    def _get_smas_relationship_from_share_server(self, share_server,
        ...
        :param fields: comma-separated list of fields to retrieve; defaults
            to SMAS relationship identity, state, and health fields. Custom
            field requests include the SMAS policy selection fields.

        :return: relationship record from the peer cluster, or ``None``.
        """
    ...
    def _get_smas_rest_client(self, share_server, vserver_name):
        """REST client for a share server's backend, scoped to its vserver.

        The smas_protection volume field (get_volume_details / patch_volume) is
        REST-only, so force a REST client even on netapp_use_legacy_client
        backends. Derive the backend from the share server's host (the config
        stanza), not self._backend_name (the logical share_backend_name, which
        is not a stanza).
        """
        backend = share_utils.extract_host(
            share_server['host'], level='backend_name')
        return data_motion.get_client_for_backend(
            backend, vserver_name=vserver_name, force_rest_client=True)
    ...
    def _verify_smas_protected(self, share, share_server):
        ...
        rest_client = self._get_smas_rest_client(share_server, src_svm)
        ...
        if (volume.get('smas_protection')
            ...
            msg = _("Share was created on a replicated share server but "
                    "is not being replicated; the share-server replica "
                    "may be out of sync. The share is in error; delete "
                    "it and retry once the share-server replica is back "
                    "in_sync.")
            raise exception.NetAppException(msg)
    ...
    def _protect_smas_volume_after_break(self, share, share_server):
        ...
        if not dest_svm:
            msg = _("Cannot protect volume: the share server's "
                    "backend details are missing the destination "
                    "vserver name.")
            raise exception.NetAppException(msg)
        share_name = self._get_backend_share_name(share['id'])
        rest_client = self._get_smas_rest_client(share_server, dest_svm)
    ...
    def delete_share(self, context, share, share_server=None):
        """Deletes a share, removing it from replication first if needed."""
    ...
    def _unprotect_smas_share(self, share, share_server):
        ...
        if protection != na_utils.SMAS_PROTECTION_PROTECTED:
            LOG.debug("Share %(share)s is not being replicated "
        ...
        if (vol_info.get('smas_protection')
                == na_utils.SMAS_PROTECTION_PROTECTED):
            msg = _("Failed to remove the share from share-server "
                    "replication; aborting destination cleanup. "
                    "Retry the delete.")
            raise exception.NetAppException(msg)
        ...
        if not dp_dest_client:
            LOG.warning("Destination cluster unreachable while removing "
                        "share %s from share-server replication; manual "
                        "cleanup of the destination volume may be needed.",
                        share['id'])
            return
        try:
    ...
    def delete_share_server_replica(self, context, share_server_replica,
        ...
        LOG.debug('Successfully deleted share server replica (SVM %s).',
                  dest_vserver)
    ...
    @na_utils.trace
    def _confirm_auto_failover_flip(
            self, active_svm, promoted_svm, active_cluster_client,
            promoted_cluster_client):
        """Returns the reversed relationship when ONTAP flipped the pair."""
        active_cluster_name = active_cluster_client.get_cluster_name()
        mediators = promoted_cluster_client.get_cluster_mediators(
            peer_cluster_name=active_cluster_name)
        try:
            data_motion.validate_mediator_reachable(
                active_cluster_name, mediators)
        except exception.NetAppException as e:
            # ONTAP cannot complete a reverse resync while the peer
            # cluster is still unreachable.
            LOG.debug('Mediator check against cluster %(cluster)s did '
                      'not pass: %(err)s.',
                      {'cluster': active_cluster_name, 'err': e})
            return None

        svm_info = (promoted_cluster_client.get_vserver_info(promoted_svm)
                    or {})
        subtype = svm_info.get('subtype')
        state = svm_info.get('state')
        if (subtype != na_utils.SVM_SUBTYPE_DEFAULT
                or state != na_utils.SVM_STATE_RUNNING):
            LOG.debug('SVM %(svm)s reports subtype=%(subtype)s, '
                      'state=%(state)s; no unplanned failover.',
                      {'svm': promoted_svm, 'subtype': subtype,
                       'state': state})
            return None

        fields = ('state,healthy,unhealthy_reason,source.path,'
                  'destination.path')
        original_rels = promoted_cluster_client.get_snapmirror_relationships(
            active_svm + ':', promoted_svm + ':', fields=fields)
        if original_rels:
            LOG.debug('SnapMirror relationship from %(src)s to %(dest)s '
                      'is still present; no unplanned failover.',
                      {'src': active_svm, 'dest': promoted_svm})
            return None

        # The promoted SVM is the source of the reversed relationship,
        # so it is only visible from the source endpoint's destination
        # list.
        reversed_rels = promoted_cluster_client.get_snapmirror_relationships(
            promoted_svm + ':', active_svm + ':', fields=fields,
            list_destinations_only=True)
        if not reversed_rels:
            LOG.debug('No reversed SnapMirror relationship from %(src)s '
                      'to %(dest)s; unplanned failover not confirmed.',
                      {'src': promoted_svm, 'dest': active_svm})
            return None

        return reversed_rels[0]
    ...
    def promote_share_server_replica(self, context, share_server_replica,
        ...
        # Protected share server instance check
        expected_names_from_manila_driver = (
    ...
    def check_for_unplanned_share_server_replica_failover(
            ...
            share_server_resources=None):
        """Detects a storage-side failover already performed by ONTAP."""
        no_promote_required = {'promote_required': False}

        protected_share_instances = (share_server_resources or {}).get(
            'protected_share_instances', [])
        active_replica = self.find_active_replica(share_server_replica_list)
        # TODO(kumart): enhance to handle multiple sync/async replica legs.
        inactive_replicas = self._find_nonactive_replicas(
            share_server_replica_list)
        if not active_replica or len(inactive_replicas) != 1:
            return no_promote_required
        inactive_sync_replica = inactive_replicas[0]

        try:
            (active_svm, promoted_svm, active_cluster_client,
             promoted_cluster_client) = (
                self._resolve_replica_pair_endpoints(
                    active_replica['share_server'],
                    inactive_sync_replica['share_server']))

            reversed_rel = self._confirm_auto_failover_flip(
                active_svm, promoted_svm, active_cluster_client,
                promoted_cluster_client)
            if not reversed_rel:
                return no_promote_required
            ...
            volume_names_to_instance_ids = (
                self._map_volume_names_to_instance_ids(
                    protected_share_instances))
            share_pool_mappings, missing_on_promoted_svm = (
                self._build_share_pool_mappings(
                    promoted_svm, promoted_cluster_client,
                    volume_names_to_instance_ids))
        except Exception as e:
            LOG.warning('Unplanned failover check did not complete for '
                        'share server replica %(id)s: %(err)s.',
                        {'id': inactive_sync_replica['id'], 'err': e})
            return no_promote_required

        for vol_name, instance_id in missing_on_promoted_svm:
            LOG.warning('Volume %(vol)s was not found on the promoted '
                        'SVM %(svm)s; host mapping skipped for share '
                        'instance %(sid)s.',
                        {'vol': vol_name, 'svm': promoted_svm,
                         'sid': instance_id})

        dm_session = data_motion.DataMotionSession()
        demoted_replica_state = (
            dm_session.map_snapmirror_state_to_replica_status(reversed_rel))

        LOG.info('Detected an unplanned failover of share server replica '
                 '%(id)s: SVM %(new)s is now the SnapMirror source and '
                 'SVM %(old)s the destination (%(state)s).',
                 {'id': inactive_sync_replica['id'], 'new': promoted_svm,
                  'old': active_svm, 'state': demoted_replica_state})
        return {
            'promote_required': True,
            'replica_list': [
                {'replica_id': inactive_sync_replica['id'],
                 'replica_state': constants.REPLICA_STATE_ACTIVE},
                {'replica_id': active_replica['id'],
                 'replica_state': demoted_replica_state},
            ],
            'share_pool_mappings': share_pool_mappings,
        }
