...
class NetAppCmodeFileStorageLibrary(object):
    ...
    def _share_server_has_replica(share_server):
        ...
        Safe base-class default: always returns False so that callers in
        the shared create_share_from_snapshot path do not need to guard
        against AttributeError when running under the single-SVM driver
        (DHSS=False) or against share servers that carry no replica list.
        The multi-SVM subclass overrides this with the real check.
        """
        return False
    ...
    def create_share_from_snapshot(self, context, share, snapshot,
        ...
        # If share is part of SVM SMAS NAS protection, reject if not in_sync.
        ...
        # SMAS NAS pre-check: volume rehost across SVMs on the same cluster
        ...
        if (src_cluster_name == dest_cluster_name and not dest_is_flexgroup
            ...
            msg = _('Could not create share %(share_id)s from snapshot '
                    ...
                    'server is protected by SM-as NAS -- volume rehost is '
        ...
        try:
            ...
            if (src_cluster_name != dest_cluster_name or dest_is_flexgroup or
                ...
                temp_share = copy.copy(dest_share)
    ...
    def _create_flexgroup_share(self, vserver_client, aggr_list, share_name,
                                ...
                                replica=False, mount_point_name=None,
                                snaplock_type=None, comment='',
                                **provisioning_options):
        ...
        job_info = self.wait_for_start_create_flexgroup(
            ...
            comment=comment, **provisioning_options)
    ...
    def _get_provisioning_options_for_share(self, share, vserver,
        ...
        if self._is_multi_protocol_share(share):
            provisioning_options['unix_permissions'] = '0777'
    ...
    def _update_access(self, helper, share, share_name, access_rules,
        ...
        for rule in access_rules:
            try:
                ...
                # multi-export shares are special case
                if not self._is_multi_protocol_share(share):
                    raise exc
            # append valid rules
    ...
    def promote_replica(self, context, replica_list, replica, access_rules,
        ...
        if is_dr and orig_active_vserver_client is not None:
    ...
    def _safe_change_replica_source(self, dm_session, replica,
        ...
        # This recovery section should also be disaster-tolerant - if we
        # can't reach the readable replica backend, mark it as error but
        # don't crash the promotion.
        try:
            ...
            # If we can't reach the readable replica backend during disaster
            # recovery, mark it as error but allow promotion to succeed
            replica['status'] = constants.STATUS_ERROR
            replica['replica_state'] = constants.STATUS_ERROR
            msg = (f"Failed to recover readable replica {replica['id']} "
                   f"after source change. Replica backend may be unreachable. "
                   f"{e}")
            LOG.exception(msg)
            return replica
